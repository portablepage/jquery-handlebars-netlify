	...
	if(user!==null){
		...
		$( document ).ajaxComplete(function() {
			...
			$('.edit').on('click', function(){
				...
				var templ_data = $('#'+templ).html();
				
				var rex = new RegExp("{{.*?}}", "g"); // "g" for global
				var fields = templ_data.match(rex);
				// console.log(matchesArray);
				
				//console.log(json);
				
				// this extracts the current post from the data
				var post = json.filter(function (json) { return json.id == id });
				post = post[0];
